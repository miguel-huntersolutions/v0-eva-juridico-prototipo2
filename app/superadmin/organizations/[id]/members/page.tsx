...
import { OrganizationMembersPage } from "@/components/superadmin/organization-members-page"
import { useProfile } from "@/hooks/use-profile"
import { useImpersonation } from "@/lib/impersonation-context"

export default function OrganizationMembersRoute({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="flex h-dvh flex-col overflow-hidden md:flex-row">
    ...
    <main className={`min-h-0 flex-1 overflow-y-auto px-4 py-4 md:px-8 md:py-8 ${isImpersonating ? "md:pt-14" : ""}`}>
      <OrganizationMembersPage organizationId={organizationId} />
    </main>
  </div>
}
